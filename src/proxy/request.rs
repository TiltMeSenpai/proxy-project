...
impl Request {
    pub fn from_request(req: hyper::Request<Body>, id: u32, channel: Sender<ProxyEvent>) -> (Self, Option<OnUpgrade>) {
        ...
        channel.send(ProxyEvent{id, event: ProxyState::RequestHead(head.clone())}).unwrap();
        (Self {
            head,
