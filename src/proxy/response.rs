...
impl Response {
    pub fn from_response(resp: hyper::Response<Body>, id: u32, channel: Sender<ProxyEvent>) -> (Self, Option<OnUpgrade>) {
        ...
        channel.send(ProxyEvent{id, event: ProxyState::ResponseHead(head.clone())}).unwrap();
        (Self {
            head,
